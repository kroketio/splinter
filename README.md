...
- extra wideness for line grids: godot/pull/106330
- default fps: 120
- default viewport width/height: 1650x1080
- default physics: jolt
...
- normal markers generate signals
- camera change markers generate `change_camera(name)` - see BlenderAnimation
...
- `get_world` - get root node, e.g. `var w: World = get_world()`
- `await sleep(5).timeout` - async wait